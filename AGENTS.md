...
- Send precommit output to a log file and search the log: `mix precommit > precommit.log 2>&1; echo $?`, then `grep -n "==> precommit\|\*\* (\|error\|warning\|failure" precommit.log`. The root run prints `==> precommit <project>` before each project, so the last such line above an error names the project. An error above the first such line belongs to the root. Git ignores `precommit.log`. The run takes minutes, and the line that names the failure is rarely among the last ones. Never pipe it to `tail`, and never rerun it to read an error.
