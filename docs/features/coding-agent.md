...
| bash command start after the group marker | the watchdog can die between the marker and the go-ahead (a `kill -9` from outside), and the child can end before or at the `exec` of bash (bash removed after it was found; `PERL_UNICODE=i` in the environment makes the child's read of the go-ahead fail) | open, ticket #70: the result is ok with `Exit code: 137`, `127`, or `255` for a command that never ran, as on master before #52. The child has no channel to say that the `exec` happened. No process is left behind on either path |
