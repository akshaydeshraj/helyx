...
- Resume: each harness issues its own session id. Helyx writes it to the transcript as a `harness_session` entry when the first harness turn starts, and passes it back on later turns and after a restart. If the harness no longer has that session, the next harness turn starts a fresh harness session, the transcript gets a new `harness_session` entry, and the TUI shows a notice that the harness lost its own context. The Helyx transcript is unaffected. Replaying the transcript into a fresh harness session is out of scope. For Claude Code, `docs/research/claude-code-stream-json.md` records what a lost harness session looks like: exit status 1 and one `result` line whose `session_id` is the id that was asked for, not a new one, and the only machine-readable sign of the reason is the text in `errors` (stderr carries the same text).
...
| tool result text | 2000 lines or 51,200 bytes of line content, on whole lines | read keeps the head and bash keeps the tail; the result says which absolute lines it shows, and a truncated head names the offset that continues the read. When the line at the kept edge (the first of a head window, the last of a tail) is over the byte cap by itself, it is cut to the cap and is the only line shown; the notice names the line and the bytes the tool kept (`, line N cut at B bytes`), and no offset reaches the rest of that line: the cut is an accepted ceiling (#50 triage), and opencode's read also marks a cut line (`docs/research/coding-tools.md`, opencode). Line numbers in a bash result count from the kept output buffer (next row). B counts the tool's own text: when the hands replace invalid bytes afterwards (row "tool result text validity"), the delivered line can be up to three times B. |
...
| Harness session entries per session file | By design (#10: the session does not write the entry yet, and it does not read the restored ids yet), one per fresh harness session: the first harness turn of a provider, and each turn after a lost harness session. `SessionFile` writes the two strings it is given and, on resume, accepts any two strings and restores the last id of each provider. The size and UTF-8 check of an id belongs where the id enters the session from the harness stream; that path is not built yet (#10) | A harness session entry whose `id`, `provider`, or `harness_session_id` is missing or not a string is a malformed file, like a bad model change. As for a model change, `parent_id`, `ts`, and extra fields are not checked, and an empty string passes; the count and the size are bounded only by the 64 MiB file limit on resume |
