...
| read tool `offset` | a positive integer, a 1-based line number. A missing or `null` offset is line 1. A float with no fraction, such as `2001.0`, is its integer, because the JSON encoders of some models send it; this holds for a float of any size. The tool has no `limit` argument, and it ignores a `limit` key like any key that the schema does not name (#75). pi and opencode both have `offset` and `limit` (`docs/research/coding-tools.md`, pi and opencode). A `limit` argument needs a line count in `Helyx.Tool.truncate/3`, which #75 does not change. This is open, ticket pending. An offset after the last line gives an empty ok result with no notice. This is older than #75 and is open, ticket pending | an error result before the file is read, `offset must be a positive integer (a 1-based line number), got K`, never a silent default. K names the kind of the value (`a string`, `an integer below 1`, `a number that is not a whole number of 1 or more`, `a boolean`, `an array`, `an object`) and never shows the value, so the size and the cost of the text do not depend on the argument; the longest text is 111 bytes. This bound is on the tool only. The session encodes the arguments of every tool call to JSON for the session file, and that cost is quadratic in the digits of a large integer argument of any tool (measured 2.5 s for 400,000 digits, under the 10 MiB limit on tool call bytes). This is older than #75 and is open, ticket pending. The other tools (`bash`, `edit`, `write`) have only string arguments, and a wrong type there is already an error result from the catch-all clause of `run/2` |
