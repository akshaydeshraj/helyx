...
- A Task that traps exits turns the trap off before any work whose time grows with an input that has no cap (an encode of the whole transcript, a replay build) and before any call that can block, because a shutdown waits for that work. A read-loop step that only parses a line is bounded by its line cap, so it can run with the trap on; a step that builds from the whole transcript is not. The rule holds on every path into the function, not only the reported one. Source: the three findings of #167 on the trap around the Claude Code start.
