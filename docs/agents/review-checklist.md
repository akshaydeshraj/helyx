...
- A plugin callback that runs in the caller of a session start or resume (a tool spec callback, `turn/0`), and any plugin code that the check of its value runs (a JSON encoder of a plugin struct), has its raise, throw, and exit contained and returned as `{:error, reason}`. Open: `id/0` in `Helyx.Provider.find/2` is not contained yet (reported with #142). Source: the Codex round 1 finding of #142.
