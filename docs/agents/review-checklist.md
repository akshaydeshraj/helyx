...
- A plugin callback that runs in the caller of a session start or resume (a tool spec callback, a tool's `check/0`, `id/0`, `turn/0`), and any plugin code that the check of its value runs (a JSON encoder of a plugin struct), has its raise, throw, and exit contained and returned as `{:error, reason}`. `id/0` is contained in `Helyx.Provider.find/2` (#153). Source: the Codex round 1 finding of #142.
