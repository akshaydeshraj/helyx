...
| Spec | A real session near the file limit must fit; 63 MiB of one-word messages does not. | Recorded in the bounds table. Accepted by the orchestrator: not a normal session. Not changed. |
...
| Spec | `Session.resume/2` can reject a saved model after `SessionFile.resume/3` repaired the file. | Out of scope: on master before #64; the bounds row claims the rule for `{:too_large, text}` only. Ticket #104. |
...
| Spec | The doc line "a client renders from the restored transcript" is false: no client can read the restored transcript. | Out of scope: on master before #64. Ticket #105. |
